...
function usePayments(params = {}) {
  ...
  const create = useCallback(async (invoiceId, paymentData) => {
...
function useRateLimit() {
  const client = getSimpleFACTClient();
  const [rateLimitInfo, setRateLimitInfo] = useState(client.getRateLimitInfo());
  useEffect(() => {
...
function useDownload() {
  ...
  const [error, setError] = useState(null);
  const downloadInvoicePDF = useCallback(async (invoiceId, filename) => {
    setDownloading(true);
    setError(null);
    try {
  ...
  const downloadBudgetPDF = useCallback(async (budgetId, filename) => {
    setDownloading(true);
    setError(null);
    try {
